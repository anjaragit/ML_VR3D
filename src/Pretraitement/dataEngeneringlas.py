...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="D:\VR3D\code\ML-VR3D\code\ML-VR3D\Pretraitement\data\zone_file\Z4\PARIS_ASS_Z4.dxf",
    ...
    parser.add_argument(
        ...
        default="D:\VR3D\code\ML-VR3D\code\ML-VR3D\Pretraitement\data\zone_file\Z4\Z4.las",
        help="Path to nuage de points topo",
    )
    parser.add_argument(
        "--h5_path",
        type=str,
        default="D:\VR3D\code\ML-VR3D\code\ML-VR3D\Pretraitement\data\h5_data\\train_VR3D_Z4_27_03_2022_H5",
...
def run_stat(dxf_file, nuage):
    ...
    # df = get_total_chambre(Isolation(stat_descr, data_stack))
    # pd.DataFrame(df).to_csv('D:\VR3D\code\ML-VR3D\code\ML-VR3D\Pretraitement\data\Z2_filtre.txt', index=False)
...
def filter_local(chmbr):
    x_data = pd.DataFrame(chmbr)
    cluster = KMeans(n_clusters=2)
    cluster.fit(x_data[2])
    label = cluster.labels_
    x_data['label'] = label
    x_data_0 = x_data[x_data['label'] == 0]
    x_data_1 = x_data[x_data['label'] == 1]
    mean_0 = x_data_0[2].mean()
    mean_1 = x_data_1[2].mean()
    if mean_0 < mean_1:
        return np.array(x_data_0.drop('label', axis=1))
    else :
        return np.array(x_data_1.drop('label', axis=1))
...
def main():
    args = parse_args()
    lasfile = File(args.nuage_path, mode='r')
    local_var = np.empty((len(lasfile), 4))
    local_var[:, 0] = lasfile.x
    local_var[:, 1] = lasfile.y
    local_var[:, 2] = lasfile.z
    local_var[:, 3] = lasfile.intensity
    ...
    array_final = training_fichier(nuage_marge, total_chambr)

    array_final = process_intensity(np.array(array_final))
