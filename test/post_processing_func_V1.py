...
def masking(binary_classes, bins):
    ...
    ind_m = np.squeeze(np.array(np.where(binary_classes[:, 4] == 1)))
    ind_nm = np.squeeze(np.array(np.where(binary_classes[:, 4] == 0)))
    ...
    if do_ratio:
        ...
        uniquem, unique_inversem, unique_countsm = np.unique(all_dm[:, 0:2], return_counts=True, return_inverse=True,
                                                             axis=0)
        uniquenm, unique_inversenm, unique_countsnm = np.unique(all_dnm[:, 0:2], return_counts=True,
        ...
        if np.array(ind).size != 0:
            ...
            for l in range(len(keypoints_sub)):
                if l == len(keypoints_sub) - 1:
                    x0, y0 = [keypoints_sub[l, 0], keypoints_sub[0, 0]], [keypoints_sub[l, 1], keypoints_sub[
                        0, 1]]  # , [edge_points[l,0,2],edge_points[l,1,2]]
                    # print("ty",x0,y0)
                    plt.plot(x0, y0, c='y')  # , marker='o'
                else:
                    x0, y0 = [keypoints_sub[l, 0], keypoints_sub[l + 1, 0]], [keypoints_sub[l, 1], keypoints_sub[
                        l + 1, 1]]  # , [edge_points[l,0,2],edge_points[l,1,2]]
                    plt.plot(x0, y0, c='y')
    plt.show()
...
def run_keypoints(array):
    ...
    if os.name == 'nt':
        ...
        command_concave_next = 'lasboundary -i ' + str(to_read) + ' -o res.txt -concavity ' + str(
            alpha_dep_next)  # +' -disjoint -holes'
    # Pour linux
    else:
        ...
        command_concave_next = 'wine lasboundary -i ' + str(to_read) + ' -o res.txt -concavity ' + str(
            alpha_dep_next)  # +' -disjoint -holes'
    ...
    for obj in objs:
        ...
        for l in range(len(c_obj)):
            if l == len(c_obj) - 1:
                ...
                x0, y0 = [c_obj[l, 0], c_obj[l + 1, 0]], [c_obj[l, 1],
                                                          c_obj[l + 1, 1]]  # , [edge_points[l,0,2],edge_points[l,1,2]]
                plt.plot(x0, y0, c='r')
    # mng = plt.get_current_fig_manager()
