...
# DATA_PATH = os.path.join(ROOT_DIR, 'data', 'EtechTopoBinaire')
# DATA_PATH_CLEAN = os.path.join(ROOT_DIR, 'data', 'EtechTopoCleaned (Discarded-Renamed-DA)')
# DATA_PATH_TEST = os.path.join(ROOT_DIR, 'data', 'test_data')

# g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'class_names.txt'))]

# "g_class2label = {cls: i for i, cls in enumerate(g_classes)}
g_class2color = {'marquage': [255, 0, 0],
                 'autre': [0, 0, 255]}
...
def room2blocks_random(data, num_point, training=True):
    ...
    for i in range(int(num_block)):
        block_data_sampled = data[num_point * i:num_point * (i + 1), ...]
        print('---------------------------------------------------')
        ...
        if block_data_sampled.shape[0] < num_point:
            if training:
                ...
                block_data_sampled = data[num_point * i:num_point * i + block_data_sampled.shape[0], ...]
...
def numpy2blocks(data, num_point, shuffle):
    # SAUVEGARDE LE SEED: ETAT ALEATOIRE DONT LES DONNÉES ONT ETE SHUFFLEES
    np.random.seed(None)
    ...
    for i in range(int(num_block)):
        ...
        if block_data_sampled.shape[0] < num_point:
            ...
            block_data_sampled = np.concatenate([block_data_sampled, dup_data], 0)
        block_data_list.append(np.expand_dims(block_data_sampled, 0))
    return np.concatenate(block_data_list, 0), seed


def numpy2blocks_with_seed(data, num_point):
    ...
    return np.array(np.concatenate(block_data_list, 0)), data.shape[0]
...
def numpy2blocks_plus_normalized(data_label, num_point, shuffle):
    """ room2block, with input filename and RGB preprocessing.
        for each block centralize XYZ, add normalized XYZ as 678 channels
    """
    data = data_label[:, 0:4]
    ...
    data_batch, seed = numpy2blocks(data, num_point, shuffle)

    new_data_batch = np.zeros((data_batch.shape[0], num_point, 7))
    for b in range(data_batch.shape[0]):
        new_data_batch[b, :, 4] -= min_room_x
        new_data_batch[b, :, 4] = data_batch[b, :, 0] / max_room_x
        new_data_batch[b, :, 5] -= min_room_y
        new_data_batch[b, :, 5] = data_batch[b, :, 1] / max_room_y
        new_data_batch[b, :, 6] -= min_room_z
        new_data_batch[b, :, 6] = data_batch[b, :, 2] / max_room_z
        minx = min(data_batch[b, :, 0])
        miny = min(data_batch[b, :, 1])
        data_batch[b, :, 0] -= (minx + 1 / 2)
        data_batch[b, :, 1] -= (miny + 1 / 2)
    new_data_batch[:, :, 0:4] = data_batch
    return new_data_batch, seed  # , label_batch
...
def room2blocks_wrapper_normalized(data_label_filename, num_point, block_size=1.0, stride=1.0,
    ...
    return room2blocks_plus_normalized(data_label, num_point, block_size, stride,
                                       random_sample, sample_num, sample_aug), seed


def numpy2blocks_normalized(data_label_filename, num_point, shuffle=False):
    # data_label = np.load(data_label_filename)
    data_label =data_label_filename.copy()
    return numpy2blocks_plus_normalized(data_label, num_point, shuffle)
